...
double apply_discount(double before_discount, double discount) {
    ...
    double discount_perc = discount / 100;
    double discount_amount = before_discount * discount_perc;
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    return static_cast<int>(std::ceil(discounted_hourly_rate * 8 * 22));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double discounted_hourly_rate = apply_discount(hourly_rate, discount);
    return (budget / discounted_hourly_rate) / 8;
}
